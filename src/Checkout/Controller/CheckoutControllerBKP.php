...
class CheckoutController extends AbstractController
{
    ...
    public function finalizarPedidoAction()
    {
        ...
        switch ($this->post('tipo_pagamento')) {
            case 'boleto':
                $this->_pagamento->efetuaPagamentoBoleto($this->post('endereco'), $this->post('hash'), $itens_pedido, $_total, $_frete, $this->post('cpf_boleto'), $idPedido);
                break;
            case 'cartao':
                $this->_pagamento->efetuaPagamentoCartao($itens_pedido, $_total, $_frete, $this->post('endereco'), $this->post('name'), $this->post('hashPagSeguro'), $this->post('tokenPagamentoCartao'), $this->post('bandeira_cartao'), $this->post('parcela'), $this->post('cpf'), $idPedido);
                break;
        }
    }
    ...
    public function cestaAction()
    {
        ...
        $data = [
            '_total' => ValidateUtil::setFormatMoney(array_sum($_total)),
            '_total_float' => array_sum($_total),
    ...
    public function cartAction()
    {
        if (isset($_POST) && sizeof($_POST) != 0) {
            ...
            if ($_hasTamanho > 0 && $_POST['tamanho'] && $_POST['tamanho'] != NULL) {
                ...
                if ($tamanhoProduto[0]['valor'] != NULL && $tamanhoProduto[0]['valor'] > 0) {
                    $product[0]['valor_venda'] = $tamanhoProduto[0]['valor'];
                }
            }
            ...
            foreach ($product as $check) {
                ...
                $cart[$check['id']] = [
                    ...
                    'valor' => $check['valor_venda'],
                    'link_compra_mercado_pago' => $check['link_compra_mercado_pago'],
                    // 'valor_unitario' => $this->dao('Core', 'Produto')->getField('valor_venda', $check['id']),
                    'valor_unitario' => $check['valor_venda'],
                    'quantidade' => 1,
                    'imagem' => $imgs[0],
                    'cor' => $_POST['cor'],
    ...
    public function finalizarAction()
    {
        ...
        if (sizeof(CarrinhoUtil::getItens('_itens')) != 0) {
            ...
            $data = [
                ...
                '_total' => ValidateUtil::setFormatMoney($_frete_and_total_carrinho['_total_carrinho']),
                '_total_float' => (double) $_frete_and_total_carrinho['_total_carrinho'],
                ...
                '_total_a_pagar' => ValidateUtil::setFormatMoney($_frete_and_total_carrinho['_total_carrinho'] + $_frete),

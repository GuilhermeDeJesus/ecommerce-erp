...
<body>
	...
	<div class="privacy">
							...
							<table class="table">
								<tbody>
									...
									<tr style="border-top: 1px solid #e5e5e5;">
										...
										<td><span class="check"><b id="_total_cart">R$ <?=$data['_total_a_pagar'];?></b></span></td>
									</tr>
									<tr style="border-top: 1px solid #e5e5e5;">
		...
		<div class="container">
			<div class="w3-container">
				...
				if($gateway == 'mercadopago' && $bybuttom == true){ ?>
				<div class="w3-row">
				...
				<?php }else if($gateway == 'mercadopago' && $bybuttom == false){ ?>
				<div class="w3-row">
				...
				<div id="Cartao" class="w3-container cartao" style="background-color: #FFF;">
                    ...
                    <div id="pgs-reprovado" style="display: none;" class="alert alert-danger" role="alert">
                      ...
                      <br>
                      <br>
                      <span id="situacao-pagamento-recusado"></span>
                      <br>
                      <br>
					...
					<div class="payments">
						<div class="col-md-6" id="div-cartao">
							<form action="?m=pagamento&c=pagamentoMP&a=checkoutTransparente" method="post" id="pagar-cartao" name="pagar-cartao">
								<div class="row-fluid">
									<div class="col-md-12">
										<input type="hidden" name="endereco" value="<?=$data['_endereco_principal']['id'];?>"> 
										<input type="hidden" name="tipo_pagamento" value="cartao">
										<input type="hidden" id="token" name="token" value="">
										<input type="hidden" class="form-control" name="_total_float" id="_total_float" value="<?=$data['_total_float'];?>">
										<input type="hidden" class="form-control" name="_total_frete" id="_total_frete" value="<?=$data['_total_frete'];?>">
										...
										<div class="form-group">
											<label>Titular </label> <input type="text" name="name" value=""
												class="form-control input-card" id="cardholderName" data-checkout="cardholderName" />
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
											<label>Número do Cartão </label> <input type="text" value=""
												class="form-control input-card" id="cardNumber" data-checkout="cardNumber"/>
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
											...
											<select class="form-control input-card"  id="installments" name="installmentsOption"></select>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label>Mês</label> 
											<select class="form-control input-card" id="cardExpirationMonth" data-checkout="cardExpirationMonth" name="expiry">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label>Ano</label>
											<select class="form-control input-card" name="cardExpirationYear" id="cardExpirationYear" data-checkout="cardExpirationYear">
									...
									<div class="col-md-2">
										<div class="form-group">
											<label>CVV </label> <input type="text"
												class="form-control input-card" id="securityCode" data-checkout="securityCode" value="" />
										</div>
									</div>
						...
						<div class="col-md-6">
							<div id="status-compra" style="display: none; text-align: center; margin-left: 30px;">
								<br> <br>
								<center>
							...
							<tbody>
								...
								<tr style="border: 1px solid #e5e5e5;">
									...
									<td class="td-price"><h6 style="text-transform: uppercase;">
											<span id="<?=$value['codigo'];?>-price">R$ <?=ValidateUtil::setFormatMoney($value['valor']);?></span>
										</h6>
									</td>
								</tr>
								<?php } ?>								
			...
			<div class="modal-content">
				<div class="modal-body modal-body-sub_agile">
					...
					<div class="modal_body_left modal_body_left1">
                		...
                		<a class="cad-endereco" href="?m=cliente&c=cliente&a=conta&redirect=checkout#parentHorizontalTab3"><i class="fas fa-map-marker-alt"></i> <b>CLIQUE AQUI PARA CADASTRAR</b></a>
                		<?php }else{ ?>
						<form action="?m=checkout&c=checkout&a=alterarEnderecoEntrega" method="post">
							<?php foreach ($data['_enderecos_cliente'] as $e) { ?>
							<div class="escolher-endereco">
								<input type="radio" name="endereco_principal" value="<?=$e['id'];?>" class="form-control radio-endereco">
	...
	<script type="text/javascript" src="public/js/mercado-pago.js"></script>
	<script src="https://www.mercadopago.com/v2/security.js" view=""></script>
	<script type="text/javascript">
		sessaoPagSeguro();
	
        Mercadopago.setPublishableKey("APP_USR-6e2b47b3-0b96-48ab-b6bb-5a8df5447ff0");
        ...
        function setPaymentMethodInfo(status, response) {
            if (status == 200) {
                ...
                var form = document.querySelector('#pagar-cartao');
                ...
                if (document.querySelector("input[name=paymentMethodId]") == null) {
                    ...
                    $("#bandeira").empty();
                    $("#bandeira").append(img);
                    amount = <?=$preference['response']['items'][0]['unit_price'];?>;
        ...
        doSubmit = false;
        addEvent(document.querySelector('#pagarCartao'), 'click', doPay);
        ...
        if (status != 200 && status != 201) {
            console.log("Erro na leitura dos parâmetros");
        }else{
            console.log(response.id);
            var form = document.querySelector('#pagar-cartao');
            document.querySelector("input[name=token]").value = response.id;
            _realizarCobranca();
        }
      };
      ...
      function setInstallmentInfo(status, response) {
        ...
        if (response.length > 0) {
            var option = new Option("Parcelar em...", '-1'),
